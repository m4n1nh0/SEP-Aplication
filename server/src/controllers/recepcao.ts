...
const normalizarDisponibilidadePaciente = (body: any): Record<string, string[]> | null => {
  ...
  for (const [dia, horas] of Object.entries(valor)) {
    if (!Array.isArray(horas)) return null;
    const lista = Array.from(new Set(
      horas
        .filter((h): h is string => typeof h === 'string' && h.trim().length > 0)
        .map(h => h.trim())
    ));
    if (lista.length) normalizada[dia] = lista;
  }
  return normalizada;
};

// ── Dashboard da recepção ─────────────────────────────────
export const dashboardRecepcao = async (_req: Request, res: Response) => {
  try {
    const [fila]: any = await pool.query(`
    ...
    const [hoje]: any = await pool.query(`
      SELECT COUNT(*) AS total,
    ...
    const [prox]: any = await pool.query(`
    ...
    const [semContato]: any = await pool.query(`
...
export const filaRecepcao = async (_req: Request, res: Response) => {
  try {
...
export const contatarPaciente = async (req: Request, res: Response) => {
  ...
  try {
    ...
    await conn.query(
      ...
      [req.params.id, tipo, assunto, mensagem || null, 'enviado', req.user!.id]
    );

    // Se atendeu, muda status para em_contato
    if (resultado === 'atendeu') {
      const [atual]: any = await conn.query('SELECT status FROM pacientes WHERE id=?', [req.params.id]);
      if (atual[0]?.status === 'aguardando') {
        ...
        await conn.query(
          ...
          [req.params.id, 'aguardando', 'em_contato', req.user!.id, 'recepcionista', `${tipo} — ${resultado}`]
        );
      }
    }
...
export const agendaHoje = async (_req: Request, res: Response) => {
  try {
    const [rows] = await pool.query(`
    ...
    logger.info('Agenda de hoje da recepcao listada', {
      total: Array.isArray(rows) ? rows.length : 0,
      primeiro_agendamento: Array.isArray(rows) && rows[0] ? {
        id: (rows as any)[0].id,
        inicio: (rows as any)[0].data_hora_inicio,
        fim: (rows as any)[0].data_hora_fim,
...
export const confirmarPresencaAdmin = async (req: Request, res: Response) => {
  try {
    await pool.query(
      "UPDATE agendamentos SET status='confirmado',atualizado_em=NOW() WHERE id=?",
      [req.params.id]
    );
    logger.info('Consulta confirmada pela recepcao', {
      agendamento_id: req.params.id,
      usuario_id: req.user?.id,
    });
    // Registra notificação automática
    const [ag]: any = await pool.query('SELECT paciente_id FROM agendamentos WHERE id=?', [req.params.id]);
    if (ag.length) {
      await pool.query(
        'INSERT INTO notificacoes (paciente_id,tipo,assunto,status,usuario_id) VALUES (?,?,?,?,?)',
        [ag[0].paciente_id, 'sistema', 'Consulta confirmada pela recepção', 'enviado', req.user!.id]
      );
    }
...
export const cancelarConsultaAdmin = async (req: Request, res: Response) => {
  const conn = await pool.getConnection();
  try {
    await conn.beginTransaction();
    const { motivo } = req.body;
    if (!motivo) return res.status(400).json({ success: false, error: 'Informe o motivo do cancelamento' });

    const [ag]: any = await conn.query(
      "SELECT id, paciente_id, status FROM agendamentos WHERE id=? AND status IN('pendente','confirmado')",
      [req.params.id]
    );
    if (!ag.length) return res.status(404).json({ success: false, error: 'Agendamento não encontrado ou já encerrado' });

    await conn.query(
      "UPDATE agendamentos SET status='cancelado_admin',notas_admin=?,atualizado_em=NOW() WHERE id=?",
      [motivo, req.params.id]
    );
    logger.info('Consulta cancelada pela recepcao', {
      agendamento_id: req.params.id,
    ...
    await conn.query(
      'INSERT INTO historico_status (paciente_id,status_anterior,status_novo,usuario_id,canal,observacao) VALUES (?,?,?,?,?,?)',
      [ag[0].paciente_id, 'agendado', 'aguardando', req.user!.id, 'recepcionista', motivo]
    );
    await conn.query(
      'INSERT INTO notificacoes (paciente_id,tipo,assunto,mensagem,status,usuario_id) VALUES (?,?,?,?,?,?)',
      [ag[0].paciente_id, 'sistema', 'Consulta cancelada', motivo, 'enviado', req.user!.id]
    );
...
export const criarPacienteCompleto = async (req: Request, res: Response) => {
  const conn = await pool.getConnection();
  try {
    await conn.beginTransaction();
    const {
      nome, cpf, email, senha_provisoria, telefone, whatsapp,
      contato_emergencia, nome_emergencia, data_nascimento, genero,
      escolaridade, ocupacao, renda_familiar, endereco, bairro,
      ...
      historico_internacao, suporte_social, risco_suicidio, risco_desc,
      outras_informacoes, urgencia, disponibilidade,
    } = req.body;
    ...
    const [rp]: any = await conn.query(
      `INSERT INTO pacientes
       ...
       VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`,
      [usuario_id, nome, cpfLimpo, email||null, telefone, whatsapp||null,
    ...
    await conn.query(
      'INSERT INTO historico_status (paciente_id,status_anterior,status_novo,usuario_id,canal,observacao) VALUES (?,?,?,?,?,?)',
      [rp.insertId, null, 'aguardando', req.user!.id, 'recepcionista', 'Cadastro presencial — triagem presencial realizada']
    );
...
export const completarCadastroPaciente = async (req: Request, res: Response) => {
  const conn = await pool.getConnection();
  try {
    await conn.beginTransaction();
    const { email, senha_provisoria, obs } = req.body;

    const [p]: any = await conn.query('SELECT * FROM pacientes WHERE id=?', [req.params.id]);
    ...
    if (!p[0].usuario_id && email) {
      const hash = await bcrypt.hash(senha_provisoria || p[0].cpf, 12);
      const [ru]: any = await conn.query(
        'INSERT INTO usuarios (nome,email,senha_hash,perfil,status_conta,email_verificado) VALUES (?,?,?,?,?,?)',
        [p[0].nome, email.toLowerCase(), hash, 'paciente', 'ativo', 1]
      );
    ...
    // Atualiza status
    await conn.query(
      'UPDATE pacientes SET status="aguardando",triagem_admin_id=?,triagem_em=NOW(),triagem_obs=?,atualizado_em=NOW() WHERE id=?',
      [req.user!.id, obs || 'Cadastro complementado pela recepção', req.params.id]
    );
    await conn.query(
      'INSERT INTO historico_status (paciente_id,status_anterior,status_novo,usuario_id,canal,observacao) VALUES (?,?,?,?,?,?)',
      [req.params.id, p[0].status, 'aguardando', req.user!.id, 'recepcionista', obs || 'Cadastro complementado']
    );
...
export const atualizarDisponibilidadePaciente = async (req: Request, res: Response) => {
  try {
    ...
    const [r]: any = await pool.query(
      'UPDATE pacientes SET disponibilidade=?,atualizado_em=NOW() WHERE id=?',
      [JSON.stringify(disponibilidade), req.params.id]
    );
    if (!r.affectedRows) return res.status(404).json({ success: false, error: 'Paciente nao encontrado' });

    res.json({ success: true, disponibilidade });
  } catch (e: any) { res.status(500).json({ success: false, error: e.message }); }
};

export const historicoContatos = async (req: Request, res: Response) => {
  try {
    const [rows] = await pool.query(`
